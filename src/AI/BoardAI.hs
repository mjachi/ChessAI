module AI.BoardAI (evaluateBoard) where
...
evaluateBoard board color
  ...
  | otherwise =
    let numberOf cc pp = length $ positionsOf board cc pp
        materialScore =
          ( 900 * (numberOf White Queen - numberOf Black Queen)
...
positionsScoreFor board cc pp scores =
  let boardOrder = if cc == White then id else reverse
      boardScores = fromList 8 8 $ boardOrder scores
   in GHC.Base.foldr (+) 0 $ map (boardScores !) (positionsOf board cc pp)
